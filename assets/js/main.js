...
function renderIntro() {
  ...
  return `
    ...
    <p>${SITE.intro}<a href="about.html">${SITE.aboutLinkText}</a>${SITE.aboutExtra}</p>
  `;
}
...
function renderHeader(active) {
  ...
  <header class="site-header">
    <div class="header-inner">
      <a class="logo" href="index.html">${SITE.title}</a>
      <nav class="main-nav">
        ${items
          .map(
            (i) =>
...
function renderFooter() {
  ...
  <footer class="site-footer">
    <div class="footer-inner">
      <a href="#">Privacy Policy</a> &nbsp;|&nbsp; &copy; ${SITE.year} ${SITE.license}
    </div>
  </footer>`;
}
  ...
  return `
    <article class="project-card">
      ...
      <div class="project-body">
        <h3><a href="project.html?slug=${encodeURIComponent(project.slug)}">${escapeHtml(project.name)}</a></h3>
        ${tech}
        <p>${escapeHtml(project.summary || "")}</p>
        <p><a class="all-posts-link" href="project.html?slug=${encodeURIComponent(project.slug)}">Read project ${ICONS.arrowRight}</a></p>
      </div>
    </article>`;
}
...
function selectHomepageItems(items, limit) {
  if (!HOME.useFeaturedFirst) return items.slice(0, limit);
